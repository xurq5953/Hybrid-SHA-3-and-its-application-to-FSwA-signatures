...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    unsigned int i;
    ALIGN(64) uint8_t seedbuf[3 * SEEDBYTES];
    ALIGN(64) uint8_t tr[CRHBYTES];
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *sk) {
    ...
    ALIGN(64) uint8_t seedbuf[2 * SEEDBYTES + 2 * CRHBYTES];
    ...
    hybrid_hash_ExpandA(mu, buf, CRHBYTES + mlen, mat, rho);
...
rej:
    ...
    for (i = 0; i < L; ++i) polyz_unpack(&y.vec[i], loop.buf[i].coeffs);
    ...
    hybrid_challenge_and_ExpandRand(sig, mu, sig, &loop, rhoprime, nonce);
    nonce += L;
    ...
    for (i = 0; i < L; i++) polyz_pack(sig + SEEDBYTES + i * POLYZ_PACKEDBYTES, &z.vec[i]);
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk) {
    ...
    ALIGN(64) uint8_t buf[K * POLYW1_PACKEDBYTES];
    uint8_t mu[CRHBYTES];
    uint8_t c[SEEDBYTES];
    ...
    hybrid_hash_pk_and_ExpandA_avx512(mu, pk, mat, rho);
    ...
    for (i = 0; i < SEEDBYTES; ++i) if (c[i] != sig[i]) return -1;
