...
int polyvecmk_uniform_eta_loop(polyvecm *u, polyveck *v, loop_queue_kg *loop,
    ...
    while (loop->size < M + K) {
        poly_rand_eta_4x(loop, pre_state, nonce, nonce + 1, nonce + 2, nonce + 3);
        nonce += 4;
    }
#if K == 2 && L == 4
    poly_rej_eta_4x(&u->vec[0], &u->vec[1], &u->vec[2], &v->vec[0], loop);
    poly_rej_eta(&v->vec[1], loop);
#elif K == 3 && L == 6
    poly_rej_eta_4x(&u->vec[0], &u->vec[1], &u->vec[2], &u->vec[3], loop);
    poly_rej_eta_4x(&u->vec[4], &v->vec[0], &v->vec[1], &v->vec[2], loop);
#elif K == 4 && L == 7
    poly_rej_eta_4x(&u->vec[0], &u->vec[1], &u->vec[2], &u->vec[3], loop);
    poly_rej_eta_4x(&u->vec[4], &u->vec[5], &v->vec[0], &v->vec[1], loop);
    poly_rej_eta_2x(&v->vec[2], &v->vec[3], loop);
#else
#error
#endif
    return nonce;
