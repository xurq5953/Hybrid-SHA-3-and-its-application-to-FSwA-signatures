...
uint16_t polyfixveclk_sample_hyperball(loop_queue *loop, polyfixvecl *y1, polyfixveck *y2, uint8_t *b, const uint8_t seed[CRHBYTES], uint16_t nonce, uint16_t
                                       rounds) {
    uint16_t ni = nonce;
        ...
        if (loop->size == 0) {
            sample_gauss_N_4x(  &samples[0],   &samples[N], &samples[2*N], &samples[3*N],
                  ...
                  &sqsum, seed,
                             ni,          ni+1,          ni+2,          ni+3,
                            N+1,           N+1,             N,             N);
            sample_gauss_N_Rnd_4x(loop,seed, ni + 4, ni + 5, ni + 6, ni + 3);
            sample_gauss_N_RejS_2x(loop,&samples[4 * N], &samples[5 * N],  &signs[4 * N / 8], &signs[5 * N / 8],
                                    &sqsum,  N, N);
            ni += 8;
        } else {
            sample_gauss_N_RejS_2x(loop,&samples[0], &samples[N],  &signs[0], &signs[N / 8],
                        &sqsum,  N + 1, N + 1);
            sample_gauss_N_4x(  &samples[2*N], &samples[3*N],&samples[4*N],   &samples[5*N],
                      &signs[2*N/8], &signs[3*N/8],&signs[4*N/8],   &signs[5*N/8],
                  &sqsum, seed,
                             ni,          ni+1,          ni+2,          ni+3,
                            N,           N,             N,             N);
            ni += 4;
...
#elif K == 3 && L == 6
        while (loop->size < L + K) {
            sample_gauss_N_Rnd_4x(loop,seed, ni, ni + 1, ni + 2, ni + 3);
            ni += 4;
        }
        sample_gauss_N_RejS_4x(loop, &samples[0], &samples[N], &samples[2 * N], &samples[3 * N], &signs[0],
                               &signs[N / 8], &signs[2 * N / 8], &signs[3 * N / 8], &sqsum, N + 1, N + 1, N, N);
        sample_gauss_N_RejS_4x(loop, &samples[4 * N], &samples[5 * N], &samples[6 * N], &samples[7 * N],
                               &signs[4 * N / 8], &signs[5 * N / 8], &signs[6 * N / 8], &signs[7 * N / 8], &sqsum, N, N,
                               N, N);
        sample_gauss_RejS_N(loop, &samples[8 * N], &signs[8 * N / 8], &sqsum, N);
#elif K == 4 && L == 7
        while (loop->size < L + K) {
            sample_gauss_N_Rnd_4x(loop,seed, ni, ni + 1, ni + 2, ni + 3);
            ni += 4;
        }
        sample_gauss_N_RejS_4x(loop,&samples[0], &samples[N], &samples[2 * N], &samples[3 * N],
                       &signs[0], &signs[N / 8], &signs[2 * N / 8], &signs[3 * N / 8],
                       &sqsum,  N + 1, N + 1, N, N);
        sample_gauss_N_RejS_4x(loop,&samples[4 * N], &samples[5 * N], &samples[6 * N], &samples[7 * N],
                       &signs[4 * N / 8], &signs[5 * N / 8], &signs[6 * N / 8], &signs[7 * N / 8],
                       &sqsum,  N, N, N, N);
        sample_gauss_N_RejS_3x(loop,&samples[8 * N], &samples[9 * N], &samples[10 * N],
                       &signs[8 * N / 8], &signs[9 * N / 8], &signs[10 * N / 8],
                       &sqsum,  N, N, N);
#else
#error
#endif
//#endif
    ...
    uint16_t ni_use = (rounds + 1) * (K+L);
      tmp[CRHBYTES + 0] = ni_use >> 0;
      tmp[CRHBYTES + 1] = ni_use >> 8;
